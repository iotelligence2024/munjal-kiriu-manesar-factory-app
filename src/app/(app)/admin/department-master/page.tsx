...
export default function DepartmentMasterPage() {
	...
	const loadItems = async () => {
		try {
			...
			const response = await fetch(`${API_BASE_URL}/api/department-master`);
			const payload = await response.json().catch(() => null);

			if (!response.ok) {
				throw new Error(payload?.message ?? "Unable to load department master.");
			}
			...
			setError(
				loadError instanceof TypeError
					...
					: loadError instanceof Error
						? loadError.message
						: "Unable to load department master."
			);
		} finally {
			setLoading(false);
		}
	...
	const handleCreate = async () => {
		...
		try {
			...
			setName("");
			setMessage(payload?.message ?? "Department created successfully.");
			await loadItems();
		} catch (createError) {
	...
	const handleDelete = async (id: string) => {
		...
		try {
			...
			const response = await fetch(`${API_BASE_URL}/api/department-master/${id}`, {
				method: "DELETE",
			});
			const payload = await response.json().catch(() => null);
			...
			setMessage(payload?.message ?? "Department deleted successfully.");
			await loadItems();
		} catch (deleteError) {
	...
	const handleEdit = async () => {
		...
		try {
			...
			const response = await fetch(`${API_BASE_URL}/api/department-master/${editId}`, {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
			...
			setEditId("");
			setEditName("");
			setMessage(payload?.message ?? "Department updated successfully.");
			await loadItems();
		} catch (updateError) {
	...
	return (
		...
		<div className={summaryShellClassName}>
			<div className="mb-3 flex items-end justify-between gap-3">
				<div className="flex-1 space-y-2">
					...
					<Input value={name} onChange={(e) => setName(e.currentTarget.value)} placeholder="Enter department name" className={inputClassName} />
				</div>
				...
				<table className="w-full min-w-[420px] table-auto border-collapse text-sm">
					<thead>
						<tr className="bg-slate-100">
							<th className={tableHeadClassName}>Department Name</th>
					...
					<tbody>
						{loading ? (
							<tr>
								<td colSpan={2} className="px-3 py-6 text-center text-slate-500">Loading departments...</td>
							</tr>
						) : items.length === 0 ? (
							<tr>
		...
		<Dialog open={editOpen} onOpenChange={setEditOpen}>
			<DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto border-[rgba(30,64,175,0.16)] bg-[linear-gradient(180deg,rgba(252,253,255,0.98),rgba(235,243,251,0.98))] sm:max-w-[min(92vw,32rem)]">
				<DialogHeader>
					<DialogTitle className="pt-3 text-2xl tracking-[-0.03em] text-[#17181d]">Edit Department</DialogTitle>
				</DialogHeader>
